...
namespace Shape
{
  ...
  {
    Point _P2; // Rechtecke haben noch eine zusätzliche Koordinate
    ...
    public override void Move(int aDx, int aDy)
    {
      ...
      base.Move(aDx, aDy);
      // um _P2 muß sich Rectangle selber kümmern
      _P2.x += aDx;
      _P2.y += aDy;
    }
  ...
  class Circle : Shape
  {
    // Kreise haben einen Radius
    int _R;

    public Circle(Point aP, int aRadius) :
      base(aP)
    {
      _R = aRadius;
    }
