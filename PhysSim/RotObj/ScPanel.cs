...
namespace MV
{
    class ScPanel : Panel
    {
        ...
        {
            gr.DrawLine(Pens.Blue, m_Xmin, 0, m_Xmax, 0);
            gr.DrawLine(Pens.Blue, 0, m_Ymin, 0, m_Ymax);
            int x;
            for (x = 0; x >= m_Xmin; x -= X_GRID)
                gr.DrawLine(m_DashPen, x, m_Ymin, x, m_Ymax);
            for (x = 0; x <= m_Xmax; x += X_GRID)
                gr.DrawLine(m_DashPen, x, m_Ymin, x, m_Ymax);
            int y;
            for (y = 0; y >= m_Ymin; y -= X_GRID)
                gr.DrawLine(m_DashPen, m_Xmin, y, m_Xmax, y);
